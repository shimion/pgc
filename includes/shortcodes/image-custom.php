...
function x_shortcode_image_custom( $atts ) {
  ...
  if ( $link == 'true' ) {
    $output = "<a href=\"{$href}\" {$title} {$target} {$tooltip_attr} {$lightbox_caption} {$lightbox_options}><img {$id} class=\"{$class}{$type}{$float}\" {$style} src=\"{$src}\" {$alt}></a>";
  } else {
...
add_shortcode( 'x_image_custom', 'x_shortcode_image_custom' );